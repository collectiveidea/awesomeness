...
namespace :backup do
  ...
  task :retrieve, :roles => :db, :only => {:primary => true} do
    get backup_file, "backups/#{backup_file(:path => false)}"
  end
  
  task :retrieve_latest, :roles => :db, :only => {:primary => true} do
    set :backup_name, latest_backup
    fetch
  end
